...
- (void)viewDidLayoutSubviews
{
    ...
    UIView *titleView = self.gk_navTitleView;//self.navigationItem.titleView;
    UIButton *backButton = self.gk_navLeftBarButtonItem.customView; //self.gk_navLeftBarButtonItem.customView;
    UIButton *cancelButton = self.gk_navRightBarButtonItem.customView;// self.gk_navRightBarButtonItem.customView;
    ...
    if (self.searchViewControllerShowMode == PYSearchViewControllerShowModePush) {
        UIButton *backButton = self.gk_navLeftBarButtonItem.customView;
...
- (UITableView *)baseSearchTableView
{
    if (!_baseSearchTableView) {
        ...
        baseSearchTableView.autoresizingMask = UIViewAutoresizingFlexibleWidth| UIViewAutoresizingFlexibleBottomMargin;
        baseSearchTableView.py_y = CGRectGetMaxY( self.gk_navigationBar.frame)
       ;
...
- (void)setCancelBarButtonItem:(UIBarButtonItem *)cancelBarButtonItem
{
    ...
    self.gk_navRightBarButtonItem = cancelBarButtonItem;
}

- (void)setCancelButton:(UIButton *)cancelButton
{
    _cancelButton = cancelButton;
    self.gk_navRightBarButtonItem.customView = cancelButton;
}
...
- (void)setSearchViewControllerShowMode:(PYSearchViewControllerShowMode)searchViewControllerShowMode
{
    ...
    if (_searchViewControllerShowMode == PYSearchViewControllerShowModeModal) { // modal
        ...
        self.gk_navRightBarButtonItem = _cancelBarButtonItem;
        self.gk_navLeftBarButtonItem = nil;
    } else if (_searchViewControllerShowMode == PYSearchViewControllerShowModePush) { // push
        self.navigationItem.hidesBackButton = YES;
        self.gk_navLeftBarButtonItem = _backBarButtonItem;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    self.baseSearchTableView.tableFooterView.hidden = YES; //0 == self.searchHistories.count || !self.showSearchHistory;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    static NSString *headerId = @"historyHeader";
    ...
    if (!headerView) {
        ...
        [emptyButton setImage:[NSBundle py_imageNamed:@"empty"] forState:UIControlStateNormal];
        [emptyButton addTarget:self action:@selector(emptySearchHistoryDidClick) forControlEvents:UIControlEventTouchUpInside];
        emptyButton.frame = CGRectMake(SCREENWIDTH-55, 0, 30, 30);
        [headerView.contentView addSubview:emptyButton];
        
    }
    headerView.textLabel.text = self.showSearchHistory && self.searchHistories.count && PYSearchHistoryStyleCell == self.searchHistoryStyle ? (self.searchHistoryTitle.length ? self.searchHistoryTitle : [NSBundle py_localizedStringForKey:PYSearchSearchHistoryText]) : nil;

    return self.searchHistories.count && self.showSearchHistory && PYSearchHistoryStyleCell == self.searchHistoryStyle ? headerView : nil;

}
- (void)tableView:(UITableView *)tableView willDisplayHeaderView:(UIView *)view forSection:(NSInteger)section{
    
    UITableViewHeaderFooterView *header = (UITableViewHeaderFooterView *)view;
    [header.textLabel setTextColor:[UIColor colorWithRed:52/255.0 green:51/255.0 blue:56/255.0 alpha:1.0]];
