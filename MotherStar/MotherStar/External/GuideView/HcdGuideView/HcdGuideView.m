...
- (UIPageControl *)pageControl {
    if (_pageControl == nil) {
        ...
        _pageControl.currentPageIndicatorTintColor = UICOLOR(@"#EFB400");
        _pageControl.frame = CGRectMake(0, 0, kHcdGuideViewBounds.size.width, 44.0f);
        _pageControl.center = CGPointMake(kHcdGuideViewBounds.size.width / 2, kHcdGuideViewBounds.size.height - 120);
    }
    return _pageControl;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == self.images.count - 1) {
        ...
        [cell.button addTarget:self action:@selector(enter) forControlEvents:UIControlEventTouchUpInside];
